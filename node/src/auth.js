...
const AUTH_BASE_DIR = (
   process.env.ZOEKT_AUTH_BASE_DIR?
   i_path.resolve(process.env.ZOEKT_AUTH_BASE_DIR):null
);
const AUTH_SESSION_TIMEOUT = parseInt(
   process.env.ZOEKT_ATUH_TIMEOUT || `${24 * 3600}`
) * 1000 || Infinity;
if (!AUTH_BASE_DIR) {
   console.warn('[!] ZOEKT_AUTH_BASE_DIR is empty: authentication module not work');
}
